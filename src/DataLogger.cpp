...
void DataLogger::begin(){
	Serial.println("DataLogger begin");
}

void DataLogger::saveError(String message, uint32_t timestamp){
	if(!isSdActive){
		return;
	}

	File dataFile = SD.open("error.err", FILE_WRITE);

	if (dataFile) {
    	Serial.println("File write");
		dataFile.println(message + ";" + String(timestamp));
		dataFile.close();
	}else {
		isSdActive = false;
	}
